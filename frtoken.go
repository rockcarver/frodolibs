...
func DetermineDeployment(tenant string, tokenId string) (string, error) {
	...
	resp1, err1 := client.R().
		...
		Get(fmt.Sprintf(oauthClientURLTemplate, tenant, "/alpha", fidcClientId))
	if resp1.StatusCode() < 200 || resp1.StatusCode() > 399 {
		if resp1.StatusCode() == 404 {
			...
			resp2, err2 := client.R().
				SetHeader("Accept-API-Version", amApiVersion).
				SetCookie(&http.Cookie{Name: cookieName, Value: tokenId,}).
				Get(fmt.Sprintf(oauthClientURLTemplate, tenant, "", forgeopsClientId))
...
func GetVersion(tenant string, tokenId string) (string, error) {
	cookieName, _ := GetCookieName(tenant)
	...
	resp1, err1 := client.R().
		SetHeader("Accept-API-Version", amApiVersion).
		SetCookie(&http.Cookie{Name: cookieName, Value: tokenId,}).
		Get(fmt.Sprintf(serverInfoURLTemplate, tenant, "version"))
...
func GetCookieName(tenant string) (string, error) {
	client := resty.New()
	resp1, err1 := client.R().Get(fmt.Sprintf(serverInfoURLTemplate, tenant, "*"))
...
func GetAccessToken(tenant string, tokenId string, realm string) (string, error) {
	...
	authorizeURL := fmt.Sprintf(authorizeURLTemplate, tenant, GetRealmUrl(realm))
	accessTokenURL := fmt.Sprintf(accessTokenURLTemplate, tenant, GetRealmUrl(realm))
	redirectURL := GetCompleteRedirectURL(tenant, redirectURLTemplate)
	cookieName, _ := GetCookieName(tenant)

	err := GetAuthCode(authorizeURL, redirectURL, codeChallenge, codeChallengeMethod, cookieName, tokenId)
	if err != nil {
...
func GetAuthCode(authorizeURL string, redirectURL string, codeChallenge string, codeChallengeMethod string, cookieName string, tokenId string) error {
	client := resty.New()
	client.SetRedirectPolicy(AuthCodeExtractRedirectPolicy())
	// client.SetDebug(true)
	resp1, err1 := client.R().
...
func Authenticate(tenant string, username string, password string, realm string) (string, string, string, error) {
	client := resty.New()
	// client.SetDebug(true)
	authURL := fmt.Sprintf("%s/json%s/authenticate", tenant, GetRealmUrl(realm))
	// fmt.Printf("%s\n", authURL)
	resp1, err1 := client.R().
	...
	if err1 == nil {
		...
		if err2 == nil {
			...
			if err3 == nil {
				...
				if err4 == nil {
					GetVersion(tenant, tokenId)
					DetermineDeployment(tenant, tokenId)
					return tokenId, deploymentType, versionString, nil
				} else {
